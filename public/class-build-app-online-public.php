...
class Build_App_Online_Public {
    ...
    public function dotapp()
    {
        ...
        $data['settings'] = array(
            ...
            'is_rtl' => is_rtl(),
            'adminUIDs' => $adminUIDs,
            'reviewVerificationRequired' => get_option( 'woocommerce_review_rating_verification_required' ),
        );
        ...
        if($data['dotapp_settings'] && $data['dotapp_settings']['multiCurrency']) {
            ...
            // For woocommerce payments plugin multicurrecny
            //$currencies = WC_Payments_Multi_Currency()->get_enabled_currencies();
            //$data['currencies'] = array_values($currencies);
    ...
    /*public function compareSlug($val1, $val2) {
    ...
    public function get_products($args = array()){
        ...
        for ($i=0; $i < 50; $i++) {
            ...
            if ( ! empty( $_REQUEST['attributes' . $i] ) && ! empty( $_REQUEST['attribute_term' . $i] ) ) {
                ...
                $attribute = sanitize_text_field($_REQUEST['attributes']) . $i;
                $attribute_term = sanitize_text_field($_REQUEST['attribute_term']) . $i;
        ...
        if(isset($_REQUEST['id']) || isset($_REQUEST['vendor'])) {
            $orderby = isset( $_REQUEST['orderby'] ) ? sanitize_sql_orderby($_REQUEST['orderby']) : 'popularity';
        } else $orderby = 'date';
        ...
        if ( ! empty( $_REQUEST['product_type'] ) ) {
            $args['type'] = sanitize_text_field( wp_unslash( $_REQUEST['product_type'] ) );
        }

        // Brand
        if ( ! empty( $_REQUEST['brand'] ) ) {
            //$brands_filter = array_filter( array_map( 'absint', explode( ',', sanitize_text_field($_REQUEST['brand']) ) ) ); // WPCS: input var ok, CSRF ok, Sanitization ok.
...
//ini_set('display_errors', '1');
            ...
            if($available_variations != null) {
                ...
                foreach ( $available_variations as $key => $value ) {
                    ...
                    foreach ( $value['attributes'] as $atr_key => $atr_value ) {
                        ...
                        $attribute_key = esc_attr( str_replace( 'attribute_', '', $atr_key ) );    
                        $attribute_type = $this->get_attribute_type($attribute_key);
                        ...
                        $values_array[] = array(
                            'name' => $this->attribute_slug_to_title($atr_key, $atr_value),
                            'slug' => $atr_value,
                            'color' => $attribute_type == 'color' ? $this->color_attribute($atr_value, $attribute_key) : null,
                            'image' => $attribute_type == 'image' ? $this->image_attribute($atr_value, $attribute_key) : null,
                        );
                        if(empty($atr_value))
                ...
                foreach ( $variation_attributes as $attribute_name => $options ) {
                    ...
                    if (!in_array('attribute_' . $attribute_name, $emptyValuesKeys)) {
                        ...
                        //Added new, delete abovce $values[] if this working properly (After 6 month from 2021 dec)
                        //$option_list = array_uintersect ( $options_array , $values_array,);

                        $option_list = array_uintersect ($options_array , $values_array, function($a, $b) {
                    ...
                    $variation_options[] = array(
                        ...
                        'option_list' => $option_list != null ? array_values($option_list) : $options_array,
    ...
    public function get_vendor_categories($id) {
        ...
        if( !empty($ids) ) {
            ...
            $categories = get_transient( 'bao-store-category-'.$unique );
            ...
            if( true ) {
                $categories = $wpdb->get_results( $wpdb->prepare( "SELECT t.term_id, t.name, tt.parent, tt.count, tt.description FROM $wpdb->terms as t
                    ...
                    AND p.post_author = %d GROUP BY t.term_id ORDER BY 'term_order'", implode(',', array_map('intval', $ids))
                ) );
                set_transient( 'bao-store-category-'.$unique , $categories );
            }
    ...
    public function updateCartQty() {
        ...
        if(isset($_REQUEST['key'])) {
            $cart_item_key = sanitize_key($_REQUEST['key']);
    ...
    public function update_order_review()
    {
       ...
       /*if ( WC()->cart->is_empty() && ! is_customize_preview() && apply_filters( 'woocommerce_checkout_update_order_review_expired', true ) ) {
        ...
        $data['csrf_token'] = sanitize_key($_SESSION['CSRF-Token']);
    ...
    public function update_user_notification(){
        ...
        if($user_id){
            ...
            if(isset($_REQUEST['fcm_token'])) {
                $fcm_token = sanitize_key($_REQUEST['fcm_token']);
    ...
    public function getProductDetail() {
        ...
        if($product = wc_get_product( $id )) {
            ...
            //Product Detail Data
            $data['customerBoughtProduct'] = wc_customer_bought_product( '', get_current_user_id(), $product->get_id() );
    ...
    public function get_wcfm_vendor_list($distance) {
        ...
        if( isset( $_REQUEST['search_data'] ) ) {
            ...
            parse_str(sanitize_text_field($_REQUEST['search_data']), $search_data);
        }
    ...
    public function get_wc_marketplace_vendor_list(){
        ...
        if ( ! empty( $_REQUEST['status'] ) ) {
            if(sanitize_text_field($_REQUEST['status']) == 'pending') $args['role'] = 'dc_pending_vendor';
            ...
            if ( empty( $addons[ $addon_key ]['field-name'] ) ) {
                ...
                foreach ( $addon['options'] as $key => $value ) {
                    if($addon['type'] == 'multiple_choice' && $addon['display'] == 'select') {
                        ...
                        $addons[$addon_key]['options'][$key]['price'] = (String)$value['price'];
                    } else {
    ...
    public function my_subscriptions( $attributes ) {
        ...
        if ( ! empty( $_REQUEST['status'] ) ) {
            $attributes['status'] = sanitize_text_field( $_REQUEST['status'] );
        }
        ...
        foreach ( $subscriptions as $subscription_id => $subscription ) {
            ...
            foreach ( $dates_to_display as $date_type => $date_title ) {
                ...
                if ( ! empty( $date ) ) {
                    $dates[] = array(
                        'title' => $date_title,
                        'date' => $subscription->get_date_to_display( $date_type ), 
                    ); 
                }     
            }
            ...
            $data[] = array(
                ...
                'date' => $subscription->get_date_to_display( 'next_payment' ),
                'payment_method' => $subscription->get_payment_method_to_display( 'customer' ),
                'order_total' => wp_kses_post( $subscription->get_formatted_order_total() ),
                'dates' => $dates,
                'notes' => $subscription->get_customer_order_notes(),
                'order' => $this->get_formatted_item_data( $order )
            ); 
        }
