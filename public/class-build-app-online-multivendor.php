...
class Build_App_Online_Multivendor
{
    ...
    public function get_wcfm_vendor_list(){
        ...
        foreach ( $stores as $store_id => $store_name ) {
            ...
            //Get bookable products from store

            $bookable_products = array();
            if(isset($_REQUEST['include_bookable'])) {
                $args = array(
                  'author' => $store_id,
                  'type'      => array('booking'),
                  'post_status'    => 'publish',
                );
                $bookable_products = $this->get_products($args);
    ...
    public function get_products($args = array()){
        ...
        if ( isset( $_REQUEST['on_sale'] ) ) {
            $on_sale_key = sanitize_text_field($_REQUEST['on_sale']) == '1' ? 'post__in' : 'post__not_in';
            $on_sale_ids = wc_get_product_ids_on_sale();
    ...
    public function store_info() {
        ...
        if(isset($_REQUEST['store_id'])) {
            $vendor_id = absint($_REQUEST['store_id']); 

            switch ($this->which_vendor()) {
                case 'dokan':
                    return $this->dokan_store_info($vendor_id);
                    break;
                case 'wcfm':
                    return $this->wcfm_store_info($vendor_id);
                    break;
                default:
                    wp_send_json(array());
            }
    ...
    public function dokan_store_info($store_id) {
        ...
        $store_data = array(
            ...
            'address' => $store_info['address'],
            'description' => $store_info['description'],
            'is_close' => dokan_is_store_open( $store_id ) ? false : true,
            'latitude' => $latitude,
            'longitude' => $longitude,
            'average_rating' => $rating['count'] == 0 ? 0 : (float)$rating['rating'],
            'rating_count' => $rating['count'],
    ...
    public function wcfm_store_info($store_id) {
